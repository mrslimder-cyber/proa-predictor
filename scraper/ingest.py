...
def run(season: str = CURRENT_SEASON, only_new: bool = True):
    ...
    with get_session() as session:
        ...
        existing_final_ids = {
            g.id for g in session.query(Game.id).filter(Game.status == "final").all()
        }
        ...
        to_scrape = [
            ...
            if not only_new or g["game_id"] not in existing_final_ids
        ]
...
def run_all_seasons(seasons: list[str] | None = None):
    ...
    solo descarga boxscores que todavía no tenga guardados.
    """
